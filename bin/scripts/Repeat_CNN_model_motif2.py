...
class Repeat_CNN_model:
    ...
    def create_model(self):
       ...
       elif self.num_filter_in_first==256: num_filter_c = [256,512,1024, 1024];
       elif self.num_filter_in_first==384: num_filter_c = [384,768,1024, 1024];
       else: num_filter_c = [512,1024,2048, 1024];
       #num_filter_c = [128,256,512, 2048]
       self.weights = {\
                      'w_c0': self.weight_variable([4, 1, self.depth, num_filter_c[0]]), \
                      'w_c1': self.weight_variable([3, 1, num_filter_c[0], num_filter_c[1]]) if self.width<7 else self.weight_variable([2, 2, num_filter_c[0], num_filter_c[1]]), \
                      ...
                      'w_f0': self.weight_variable([13*self.width*num_filter_c[1], num_filter_c[3]]) if self.width<7 else self.weight_variable([13*int(self.width/2+0.5)*num_filter_c[1], num_filter_c[3]]), \
       ...
       self.cn_flat = tf.reshape(self.cn1, [-1, self.weights['w_f0'].get_shape().as_list()[0]]);
...
if __name__=='__main__':
   c_r3cnn = Repeat_CNN_model(4, 50, 3, 3, 'cpu');
